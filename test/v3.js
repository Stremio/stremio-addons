...
tape('detectFromURL: detect and use manifest.json URL', function(t) {
	...
	const detectFromURL = promisify(AddonClient.detectFromURL)
	let addon 
	let addonGet
	detectFromURL('ipfs://QmTQTixUrtf9E4fasjes5Jb1o956FF6xqSsXnwrc5GLKeB/manifest.json')
	.then(function(res) {
		t.ok(res.addon, 'addon is ok')
		t.ok(res.addon.manifest, 'manifest is ok')
		t.deepEqual(res.addon.manifest.catalogs, ['top'], 'catalogs is right')
		...
		addon = res.addon
		addonGet = promisify(res.addon.get)

		return addonGet('catalog', 'top')
	})
	.then(function(resp) {
		t.ok(Array.isArray(resp), 'response is an array')
		return addonGet('meta', resp[0].type, resp[0].id)
	})
	.then(function(meta) {
		t.ok(meta.id, 'meta has id')

		//return addonGet('stream', 'movie', parseInt(Math.random()*1000))
